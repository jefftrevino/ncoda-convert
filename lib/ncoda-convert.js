...
export default {
  ...
  consumeBlankRuntime(blankScriptRuntime) {
    this.blankScriptRuntime = blankScriptRuntime
    console.log(blankScriptRuntime)
    return new Disposable(() => {
      this.blankScriptRuntime = null;
    });
  },
  activatePackage(packageName) {
    ...
    if (scriptIsLoaded) {
      thePackage = atom.packages.loadPackage(packageName)
        ...
        throw(new Error("Packaged not installed: ${packageName}"))
    }
  ...
  runBlank(){
    ...
    console.log(this.blankScriptRuntime)
    this.blankScriptRuntime.execute()
  }
}
